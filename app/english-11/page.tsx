...
export default async function Page() {
  ...
  const units: Item[] = (m.units ?? []).map((u: any) => ({
    ...
    slug: `unit/${u.index}`,
    imageUrl: `/data/images/unit-11-${u.index}.jpg`, // ảnh local trong public/images
    type: 'unit',
  }));
  ...
  const reviews: Item[] = (m.reviews ?? []).map((r: any) => ({
    ...
    slug: `review/${r.index}`,
    imageUrl: `/data/images/review-11-${r.index}.jpg`, // bạn có thể thay theo pattern ảnh của bạn
    type: 'review',
  }));
