...
export const metadata = { title: 'Tiếng Anh 12 – Global Success' };

export default async function Page() {
  // manifest dạng { book, units, reviews? }
  const mod = await import('../../public/data/english12_manifest.json');
  ...
  const units: Item[] = (m.units ?? []).map((u: any) => ({
    ...
    slug: `unit/${u.index}`,
    imageUrl: `/data/images/unit-12-${u.index}.jpg`, // ảnh local trong public/images
    type: 'unit',
  }));
  ...
  const reviews: Item[] = (m.reviews ?? []).map((r: any) => ({
    ...
    slug: `review/${r.index}`,
    imageUrl: `/data/images/review-12-${r.index}.jpg`, // bạn có thể thay theo pattern ảnh của bạn
    type: 'review',
  }));
  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <h2 className="text-2xl md:text-3xl font-semibold mb-6">Khóa học Tiếng Anh 12</h2>

        <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {items.map((it, idx) => (
            <Link
              key={idx}
              href={`/english-12/${it.slug}`}
              className="group rounded-2xl overflow-hidden shadow-sm ring-1 ring-gray-200 hover:shadow-lg transition"
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                <Image
                  src={it.imageUrl || `/data/images/unit-12-${idx + 1}.jpg`} // ✅ không dùng 'u' nữa
                  alt={it.title}
                  fill
